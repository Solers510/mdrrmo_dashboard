...
def get_active_event_summary() -> dict[str, object] | None:
    with SessionLocal() as session:
        ...
        if "alert_level" not in row_dict:
            if "alert_code" in row_dict:
                row_dict["alert_level"] = row_dict["alert_code"]
            elif "current_alert_level_id" in row_dict:
                levels = fetch_alert_levels(session)
                for lvl in levels:
                    if lvl["id"] == row_dict["current_alert_level_id"]:
                        row_dict["alert_level"] = lvl["code"]
                        break
                else:
